...
public sealed class ContactInfoTests : TestBase
{
    ...
    public async Task ContactSection_DisplaysConfiguredEmailAddress()
    {
        ...
        await _homePage.ClickNavItemAsync(contactItem);
        await _homePage.AssertPanelIsActiveAsync(contactItem);

        var actualEmail = await _homePage.GetContactEmailAddressAsync();

        Assert.That(
            actualEmail,
            Is.EqualTo(expectedEmail).IgnoreCase,
            $"Contact email displayed on site ('{actualEmail}') does not match the expected value ('{expectedEmail}') configured in appsettings.json. URL={Page.Url} | Title={await Page.TitleAsync()}");
    }
    ...
    public async Task CheckPhoneNumberIsAccurate()
    {
        ...
        var actualPhone = await _homePage.GetContactPhoneNumberAsync();

        Assert.That(
            actualPhone,
            Is.EqualTo(expectedPhoneNumber).Or.EqualTo(localFormat).Or.EqualTo(intlFormat),
            $"Contact phone displayed on site ('{actualPhone}') does not match the expected value ('{expectedPhoneNumber}') configured in appsettings.json. URL={Page.Url} | Title={await Page.TitleAsync()}");
    }
}